package tracker.digitalpersona;
...
public class Enrollment extends JPanel implements ActionListener {
    ...
    public MySQLC mysqlc;
    public RoomSelect So;
    int count = 0;
    ...
    public class EnrollmentThread extends Thread implements Engine.EnrollmentCallback {
        ...
        public String m_rollnumber;
        private boolean m_bCancel;
        private RoomSelect SSo;

        public EnrollmentThread(Reader reader, ActionListener listener, String m) {
            m_reader = reader;
            m_listener = listener;
            m_rollnumber = m;
        ...
        public EnrollmentThread(Reader reader, ActionListener listener, String m, RoomSelect So) {
            m_reader = reader;
            m_listener = listener;
            m_rollnumber = m;
            SSo = So;
        ...
        public Engine.PreEnrollmentFmd GetFmd(Fmd.Format format) {
            ...
            while (null == prefmd
                ...
                m_capture
                        = new CaptureThread(m_reader, false, Fid.Format.ANSI_381_2004, Reader.ImageProcessing.IMG_PROC_DEFAULT);
    ...
    public Enrollment(Reader reader) {
        m_reader = reader;
        m_bJustStarted = true;
        m_enrollment = new EnrollmentThread(m_reader, this, "testing");
    ...
    public Enrollment(Reader reader, RoomSelect So) {
        m_reader = reader;
        m_bJustStarted = true;
        this.So = So;

        m_enrollment = new EnrollmentThread(m_reader, this, "testing", So);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals(ACT_BACK)) {
            ...
            if (e.getActionCommand().equals(EnrollmentThread.ACT_PROMPT)) {
                if (m_bJustStarted) {
                    m_text.append("Enrollment started for ");
                ...
                if (null != evt.capture_result) {
                    if (evt.capture_result.image != null) {
                        m_imagePanel.showImage(evt.capture_result.image, barcode + "-" + count);
                        count++;

                        //    m_imagePanel.saveBack(this.username_text.getText());
                    }
                }
                ...
                if (null == evt.exception) {
                    String str
                            = String.format("    Enrollment template created, size: %d\n\n\nPlease save to file or verify.",
                                    evt.enrollment_fmd.getData().length);
                    enrollmentFMD = evt.enrollment_fmd;
                    saveTemplate(this.enrollmentFMD.getData());
                    //                    String s = new String(this.enrollmentFMD.getData());
                    ...
                    // Delay needs to be added 3000 sec <-
                    //here we are disposing the Enrollment Form \\
                    m_dlgParent.dispose();
    ...
    private void saveTemplate(final byte[] data) {

        Thread save = new Thread(new Runnable() {

            @Override
            public void run() {
                File mkDir = new File(new Prop.GetStorageLocations().getTemplateFolderName());
                if (mkDir.exists() == false) {
                    mkDir.mkdir();

                }
                File file = new File(new Prop.GetStorageLocations().getTemplateFolderName()+"/"+barcode + ".fmt");

                //    JFileChooser fc = new JFileChooser(new File(barcode));
                ...
                try {
                    ...
                    // s = new sqlite();
                    mysqlc = new MySQLC();
                    ////////////////////////// Barcode & Template Binary Data
                     mysqlc.writeDataBase_fingerprint(barcode, data);
                    //  s.addCandidateTemplateToDB(barcode, data);
                    //  s.terminate();
                    So.checkFmtExisits();
                    So.markwhenBothPresent();
    ...
    private void savetoSqlite() throws ClassNotFoundException, SQLException {
    }
    ...
    public static Fmd Run(Reader reader, String barcode) {
        setBarcode(barcode);
        JDialog dlg = new JDialog((JDialog) null, "Enrollment of " + barcode, true);
        Enrollment enrollment = new Enrollment(reader);
        enrollment.doModal(dlg);
        return enrollment.enrollmentFMD;
    }

    public static Fmd Run(Reader reader, String barcode, RoomSelect So) {
        setBarcode(barcode);
        JDialog dlg = new JDialog((JDialog) null, "Enrollment of " + barcode, true);
        Enrollment enrollment = new Enrollment(reader,So);
        enrollment.doModal(dlg);
        return enrollment.enrollmentFMD;
    }
}
